...
export const OrderBook = observer(({ symbol }: Props) => {
  ...
  const AskRow = ({ level, maxQty }: { level: OrderBookLevel; maxQty: number }) => {
    ...
    return (
      <div className="bg-[#1a1d27] rounded-xl overflow-hidden text-slate-200">
        {/* Header */}
        <div className="grid grid-cols-2 px-2 py-1.5 text-[11px] text-slate-500 border-b border-[#2a2d3a]">
        ...
        {/* Spread */}
        <div className="flex justify-center py-1 text-[11px] text-slate-500 border-t border-[#2a2d3a]">
          Spread: {spread}
        </div>
      </div>
    );
